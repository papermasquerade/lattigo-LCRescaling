...
func (r *Ring) ModupFromQLMinus1NoNTT(c0 *Poly, maxLevel int, N int) {

	nowLevel := c0.Level()

	if nowLevel < maxLevel {
		c0.Coeffs = append(c0.Coeffs, make([][]uint64, maxLevel-nowLevel)...)
		for i := nowLevel + 1; i < maxLevel+1; i++ {
			c0.Coeffs[i] = make([]uint64, N)
		}
	}
	...
	l := len(r.Modulus) - 1
	ql := r.Modulus[l]
	bredql := BRedParams(ql)

	var QL1 uint64 = 1
	for _, qi := range r.Modulus[:l] {
		QL1 = BRed(QL1, qi, ql, bredql)
	}

	prodQiModQL := make([]uint64, len(r.Modulus))
	invQi := make([]uint64, len(r.Modulus))
	for i := range invQi {
		invQi[i] = 1
		prodQiModQL[i] = 1
	}

	for i, qi := range r.Modulus[:l] {
		for j, qj := range r.Modulus {
			if i == j || j == l {
				continue
			}

			invQi[i] = BRed(invQi[i], qj, qi, bredparams[i])
			prodQiModQL[i] = BRed(prodQiModQL[i], qj, ql, bredql)
		}
		invQi[i] = ModInv(invQi[i], qi)
	}

	// coefi for indexing coefficienct of polynomials
	for coefi := 0; coefi < N; coefi++ {
		var rL uint64 = 0
		var z float64 = 0
		...
		for i := 0; i < l; i++ {
			...
			// 	invQi = BRed(invQi, qj, qi, bredparams[i])
			//
			// 	// prod_(j eq.not i) q_j mod q_L
			// 	prodQiModQL = BRed(prodQiModQL, qj, ql, bredql)
			// }
			// invQi = ModInv(invQi, qi)
			// invQi = BRed(invQi, c0.Coeffs[i][coefi], qi, bredparams[i])
			//
			// if invQi > (qi >> 1) {
			// 	z -= float64(qi-invQi) / float64(qi)
			// 	rL += ql - BRed(prodQiModQL, qi-invQi, ql, bredql)
			// } else {
			// 	z += float64(invQi) / float64(qi)
			// 	rL += BRed(prodQiModQL, invQi, ql, bredql)
			// }
			//
			invQic0 := BRed(invQi[i], c0.Coeffs[i][coefi], qi, bredparams[i])
			if invQic0 > (qi >> 1) {
				z -= float64(qi-invQic0) / float64(qi)
				rL += ql - BRed(prodQiModQL[i], qi-invQic0, ql, bredql)
			} else {
				z += float64(invQic0) / float64(qi)
				rL += BRed(prodQiModQL[i], invQic0, ql, bredql)
			}

			rL = BRedAdd(rL, ql, bredql)
		}

		z = math.Round(z)
		if z < 0 {
			rL += BRed(QL1, uint64(-z), ql, bredql)
		} else {
			rL += ql - BRed(QL1, uint64(z), ql, bredql)
		}
		c0.Coeffs[l][coefi] = BRedAdd(rL, ql, bredql)
	}

}

// produce the final step of level free
func (r *Ring) ModupFromQLMinus1(c0 *Poly, maxLevel int, N int) {

	r.InvNTTLvl(c0.Level(), c0, c0)
	nowLevel := c0.Level()
	...
	r.NTTLvl(c0.Level(), c0, c0)

}

func (r *Ring) PolyToBigintCenteredDirectly(p1 *Poly, coeffs []*big.Int) {
	N := r.N
	for i := 0; i < N; i++ {
		coeffs[i] = big.NewInt(0)
	}

	mulmods := big.NewInt(1)
	bigmods := make([]*big.Int, 0)
	halfbigmods := make([]*big.Int, 0)
	for _, modulus := range r.Modulus {
		mod := big.NewInt(int64(modulus))
		halfmod := big.NewInt(0)
		halfmod.Rsh(mod, 1)

		mulmods.Mul(mulmods, mod)
		bigmods = append(bigmods, mod)
		halfbigmods = append(halfbigmods, halfmod)
	}

	mulModNoQi := make([]*big.Int, 0)
	mulModNoQiInvQi := make([]*big.Int, 0)
	for _, modulus := range bigmods {
		tmp := big.NewInt(0)
		tmp.Div(mulmods, modulus)
		// tmp.Mod(tmp, modulus)
		mulModNoQi = append(mulModNoQi, tmp)

		modinv := big.NewInt(0)
		modinv.Div(mulmods, modulus)
		modinv.Mod(modinv, modulus)
		modinv.ModInverse(modinv, modulus)
		halfMod := big.NewInt(0).Set(modulus)
		halfMod.Rsh(halfMod, 1)
		if modinv.Cmp(halfMod) >= 1 {
			modinv.Sub(modinv, modulus)
		}

		mulModNoQiInvQi = append(mulModNoQiInvQi, modinv)
	}
	for j := range p1.Coeffs[0] {
		for i := range p1.Coeffs {
			tmp := NewUint(p1.Coeffs[i][j])
			if tmp.Cmp(halfbigmods[i]) >= 0 {
				tmp.Sub(tmp, bigmods[i])
			}

			tmp.Mul(tmp, mulModNoQi[i])
			tmp.Mul(tmp, mulModNoQiInvQi[i])
...
func (r *Ring) PolyToBigintDirectly(p1 *Poly, coeffs []*big.Int) {
	N := r.N
	for i := 0; i < N; i++ {
		coeffs[i] = big.NewInt(0)
	}
	...
	for j := range p1.Coeffs[0] {
		for i := range p1.Coeffs {
			tmp := big.NewInt(int64(p1.Coeffs[i][j]))
			tmp.Mul(tmp, mulModNoQi[i])
			tmp.Mul(tmp, mulModNoQiInvQi[i])
...
func (r *Ring) PrintHeadNTT(pntt *Poly, N int) {
	...
	r.PolyToBigintCenteredLvl(p.Level(), p, coeffs)
	fmt.Println(coeffs[:N])
}

func (r *Ring) PrintHead(pntt *Poly, N int) {
	coeffs := make([]*big.Int, r.N)
	for i := 0; i < r.N; i++ {
		coeffs[i] = big.NewInt(0)
	}

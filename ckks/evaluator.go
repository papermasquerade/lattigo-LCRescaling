...
type Evaluator interface {
	...
	MergeCTSMatRotKeys(matrices []LinearTransform, kgen KeyGenerator, sk *rlwe.SecretKey, rotations []int, lvlFree []bool)
	PermuteNTTIndexOfRotation(int) []uint64
	// Addition
	...
	LinearTransformWithPrecomputedMatRotKeys(ctIn *Ciphertext, linearTransform interface{}, key *rlwe.RotationKeySet, ctOut []*Ciphertext, levelFree bool)
	...
	MultiplyByDiagMatrixPrecomputed(ctIn *Ciphertext, matrix LinearTransform, rotKeys *rlwe.RotationKeySet, PoolDecompQP []rlwe.PolyQP, ctOut *Ciphertext, levelFree bool)
	...
	MatRotKeys(i int) *rlwe.RotationKeySet
	InitialMatRotKeys()
	AppendNewMatRowKeys(matrotkey *rlwe.RotationKeySet)

	MarshallLevelFreeAuxCt(filename string)
	UnmarshallLevelFreeAuxCt(filename string)
}
...
type evaluator struct {
	...
	*rlwe.KeySwitcher
	...
	matRotKeys []*rlwe.RotationKeySet
	// levelFreeAuxCt0 []rlwe.PolyQP
	// levelFreeAuxCt1 []rlwe.PolyQP
	//
	levelFreeAuxCt map[uint64][]rlwe.PolyQP
}

func (eval *evaluator) InitialMatRotKeys() {
	eval.matRotKeys = make([]*rlwe.RotationKeySet, 0)
}

func (eval *evaluator) AppendNewMatRowKeys(matrotkey *rlwe.RotationKeySet) {
	eval.matRotKeys = append(eval.matRotKeys, matrotkey)
}

func (eval *evaluator) MatRotKeys(i int) *rlwe.RotationKeySet {
	return eval.matRotKeys[i]
}

func (eval *evaluator) MarshallLevelFreeAuxCt(filename string) {
	datalen :=
		eval.RingQP().NewPoly().P.GetDataLen(true) + eval.RingQP().NewPoly().Q.GetDataLen(true)
	buffer := make([]byte, datalen, datalen)
	if eval.levelFreeAuxCt != nil {
		os.Create(filename + "-rot")
		fp, err := os.OpenFile(fmt.Sprintf("%s-rot", filename), os.O_TRUNC|os.O_WRONLY, os.ModeAppend)
		if err != nil {
			panic(err)
		}

		for k := range eval.levelFreeAuxCt {
			for l, aux_ct := range eval.levelFreeAuxCt[k] {
				if _, err := fp.WriteString(fmt.Sprintf("%d %d\n", k, l)); err != nil {
					panic(err)
				}

				pt, err := aux_ct.WriteTo(buffer)
				if err != nil {
					panic(err)
				}

				os.WriteFile(fmt.Sprintf("%s-%d-%d", filename, k, l), buffer[:pt], 0644)
			}
		}
...
func (eval *evaluator) UnmarshallLevelFreeAuxCt(filename string) {
	fp, err := os.Open(fmt.Sprintf("%s-rot", filename))
	if err != nil {
		panic(err)
	}
	defer fp.Close()
	...
	for scanner.Scan() {
		row := scanner.Text()
		nums := strings.Split(strings.TrimSpace(row), " ")
		kk, err := strconv.Atoi(nums[0])
		if err != nil {
			panic(err)
		}

		ll, err := strconv.Atoi(nums[1])
		if err != nil {
			panic(err)
		}
		...
		_, flag := eval.levelFreeAuxCt[k]
		if !flag {
			// naive implementation, one could store and sort the polyqp for robustness
			eval.levelFreeAuxCt[k] = make([]rlwe.PolyQP, 0)
		}

		// it is assumed that in the `filename-rot`, the `l`s are sorted and started from 0
		ct_q_data, err := os.ReadFile(fmt.Sprintf("%s-%d-%d", filename, k, l))
		if err != nil {
			panic(err)
		}
		ct := rlwe.PolyQP{}
		ct.DecodePolyNew(ct_q_data)

		eval.levelFreeAuxCt[k] = append(eval.levelFreeAuxCt[k], ct)
	}
}

func (eval *evaluator) MergeCTSMatRotKeys(
	matrices []LinearTransform,
	kgen KeyGenerator,
	sk *rlwe.SecretKey,
	rotations []int,
	lvlConserved []bool,
	...
	for _, mat := range matrices {
		lens = append(lens, len(mat.Vec))
	}
	fmt.Println("gonna merge", lens, "rotation keys")
	for i, mat := range matrices {
		mat_vecs := make(map[uint64]rlwe.PolyQP)
		// if i == 0 {
		// 	eval.matRotKeys = append(eval.matRotKeys, nil)
		// 	continue
		// }
		...
		for k := range mat.Vec {
			...
			// index := eval.PermuteNTTIndexOfRotation(-k)
			invGalEl := eval.params.InverseGaloisElement(galEl)
			index := ringQ.PermuteNTTIndex(invGalEl)
			// index := eval.PermuteNTTIndexOfRotation(-k)
			matVec := ringQP.NewPoly()

			ringQP.PermuteNTTWithIndexLvl(levelQ, levelP, mat.Vec[k], index, matVec)
			mat_vecs[galEl] = matVec
		}

		if lvlConserved[i] {
			// eval.matRotKeys = append(eval.matRotKeys, nil)
			eval.matRotKeys = append(eval.matRotKeys, kgen.GenRotationKeysForRotationsApplyFunc(rotations, true, sk, mat_vecs, eval.levelFreeAuxCt, levelQ))
			// continue
		} else {
			eval.matRotKeys = append(eval.matRotKeys, kgen.GenRotationKeysForRotationsApplyFunc(rotations, true, sk, mat_vecs, nil, levelQ))
		}
	}
}

func (eval *evaluator) PermuteNTTIndexOfRotation(k int) []uint64 {
	galEl := eval.params.GaloisElementForColumnRotationBy(k)
	return eval.permuteNTTIndex[galEl]
}
...
func (eval *evaluator) DropLevelNew(ct0 *Ciphertext, levels int) (ctOut *Ciphertext) {
	// fmt.Println("*************Droplevel new called")
	ctOut = ct0.CopyNew()
...
func (eval *evaluator) DirectRescale(ctIn *Ciphertext, ctOut *Ciphertext) (err error) {
	...
	level := ctIn.Level()
	for i := range ctOut.Value {
		ringQ.DivRoundByLastModulusManyNTTLvl(level, 1, ctIn.Value[i], eval.poolQMul[0], ctOut.Value[i])
...
func (eval *evaluator) ModupFromQLMinus1(ct *Ciphertext) (err error) {
	...
	for u := range ct.Value {
		ct.Value[u].Coeffs = append(ct.Value[u].Coeffs, make([][]uint64, eval.params.MaxLevel()-nowLevel)...)
		for i := nowLevel + 1; i < eval.params.MaxLevel()+1; i++ {
			ct.Value[u].Coeffs[i] = make([]uint64, eval.params.N())
		}
	}

...
type Evaluator interface {
	MergeCTSMatRotKeys(matrices []ckks.LinearTransform, kgen ckks.KeyGenerator, sk *rlwe.SecretKey, rotations []int, lvlFree []bool)
	// =======================================
	...
	DirectRescale(ctIn *ckks.Ciphertext, ctOut *ckks.Ciphertext) (err error)
	...
	CoeffsToSlotsPrecomputed(ctIn *ckks.Ciphertext, ctsMatrices EncodingMatrix, ctReal, ctImag *ckks.Ciphertext)
	// =================================================
	...
	Dft(ctIn *ckks.Ciphertext, plainVectors []ckks.LinearTransform, ctOut *ckks.Ciphertext)
	LinearTransformWithPrecomputedMatRotKeys(ctIn *ckks.Ciphertext, linearTransform interface{}, key *rlwe.RotationKeySet, ctOut []*ckks.Ciphertext, levelFree bool)
...
func (eval *evaluator) Dft(ctIn *ckks.Ciphertext, plainVectors []ckks.LinearTransform, ctOut *ckks.Ciphertext) {
	eval.dft(ctIn, plainVectors, ctOut)
}

// func (eval *evaluator) MergeCTSMatRotKeys(matrices []ckks.LinearTransform, kgen ckks.KeyGenerator, sk *rlwe.SecretKey, rotations []int) {
// 	eval.Evaluator.MergeCTSMatRotKeys(matrices, kgen, sk, rotations)
// }

func (eval *evaluator) PermuteNTTIndexOfRotation(k int) []uint64 {
	return eval.Evaluator.PermuteNTTIndexOfRotation(k)
}
...
// If the packing is sparse (n < N/2), then returns ctReal = Ecd(vReal || vImag) and ctImag = nil.
// If the packing is dense (n == N/2), then returns ctReal = Ecd(vReal) and ctImag = Ecd(vImag).
func (eval *evaluator) CoeffsToSlotsPrecomputed(ctIn *ckks.Ciphertext, ctsMatrices EncodingMatrix, ctReal, ctImag *ckks.Ciphertext) {
	zV := ctIn.CopyNew()

	eval.dftPrecomputed(ctIn, ctsMatrices.matrices, zV)
	...
	// Imag part
	fmt.Println("zv level:", zV.Level(), ", ctReal:", ctReal.Level())
	eval.Sub(zV, ctReal, tmp)
	eval.DivByi(tmp, tmp)
...
func (eval *evaluator) SlotsToCoeffsNew(ctReal, ctImag *ckks.Ciphertext, stcMatrices EncodingMatrix) (ctOut *ckks.Ciphertext) {
	...
	if ctReal.Level() < stcMatrices.LevelStart || (ctImag != nil && ctImag.Level() < stcMatrices.LevelStart) {
		fmt.Errorf("levels:", ctReal.Level(), ctImag.Level(), stcMatrices.LevelStart)
...
func (eval *evaluator) dftPrecomputed(ctIn *ckks.Ciphertext, plainVectors []ckks.LinearTransform, ctOut *ckks.Ciphertext) {
...
func (eval *evaluator) EvalModNew(ct *ckks.Ciphertext, evalModPoly EvalModPoly) *ckks.Ciphertext {
	...
	if ct.Level() > evalModPoly.LevelStart() {
		...
		// fmt.Println("level:", ct.Level(), " --> ", evalModPoly.LevelStart())
		// eval.MultByConst(ct, eval.params.RingQ().Modulus[ct.Level()], ct)

		// minScale := ct.Scale
		// ct.Scale *= float64(eval.params.RingQ().Modulus[ct.Level()])
		// eval.Rescale(ct, minScale, ct)

		// fmt.Println("level:", ct.Level())

...
func (ecdMat *EncodingMatrix) Matrices() []ckks.LinearTransform {
	return ecdMat.matrices
}
...
type EncodingMatrixLiteral struct {
	...
	LevelConserved      bool // flag that indicates whether uses LCRescaling
}
...
func (mParams *EncodingMatrixLiteral) Rotations(logN, logSlots int) (rotations []int) {
	...
	// **by adjusting BSGSRatio to very big, e.g. dslots here, then N1 can be set to 1**
...
func NewHomomorphicEncodingMatrixFromLiteral(mParams EncodingMatrixLiteral, encoder ckks.Encoder) EncodingMatrix {
	...
	// fmt.Println("generate", depth, "matrices")
	// if mParams.LevelFree {
	// 	depth++
	// }
	// fmt.Println("generate", depth, "matrices")
	...
	if mParams.LevelConserved {
		for i := range ctsLevels[1:] {
			ctsLevels[i+1] += 1
		}
	}
...
// given logN, logSlots and maxDepth, decompose DFT into layers of rotations
func computeBootstrappingDFTIndexMap(logN, logSlots, maxDepth int, ltType LinearTransformType, bitreversed bool) (rotationMap []map[int]bool) {
	...
	lens := make([]int, 0)
	for i := range rotationMap {
		lens = append(lens, len(rotationMap[i]))
	}
	return
}
